...
function minimumNumberGame(nums) {
    ...
    while (nums.length > 0) {
        ...
        for (let i = 1; i < nums.length; i++) {
            if (nums[i] < nums[aliceMinIndex]) {
                aliceMinIndex = i;
            }
        }
        // nums 배열 변경
        const aliceMin = nums.splice(aliceMinIndex, 1)[0];

        // 배열 길이가 1이면
        if (nums.length === 0) {
            answer.push(aliceMin);
            break;
        }
        ...
        let bobMinIndex = 0;
        for (let i = 1; i < nums.length; i++) {
            if (nums[i] < nums[bobMinIndex]) {
                bobMinIndex = i;
            }
        }
        // nums 배열 변경
        const bobMin = nums.splice(bobMinIndex, 1)[0];
